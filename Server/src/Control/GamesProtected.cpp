...
int GamesProtected::createGameAndAddPlayer(const ResponseInitialStateDTO &response,
                    Socket &sktPeer, const std::string &playerName) {
...
int GamesProtected::addPlayer(const ResponseInitialStateDTO &response, Socket &sktPeer, const std::string &playerName) {
    ...
    if (answer == SUCCESS) {
        std::cerr << "[GamesProtected]: Se unio: " + playerName +
        " con exito a la partida : " + response.getGameName() + "\n";
    } else {
        std::cerr << "[GamesProtected]: ERROR No se agrego al jugador " +
