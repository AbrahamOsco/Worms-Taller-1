...
WeaponSight::WeaponSight(const float &rayLength, const float &rayAngle, const GameParameters& parameters)
        : rayLength(rayLength), rayAngle(rayAngle), gameParameters(parameters), direction(RIGHT) {
}
...
GameObject* WeaponSight::getBodyCollidesWithRayCast(b2World *world, const b2Vec2& positionWorm,
    ...
    if ( gameObject != nullptr && gameObject->getEntityType() == ENTITY_WORM ) {
        return gameObject;
    }
    return nullptr;
}
...
WeaponSightDTO WeaponSight::getWeaponSightDTO(const b2Vec2 &positionWorm, const Direction &directionCurrent) {
    ...
    return WeaponSightDTO(SHOW_SIGHT, p2.x * gameParameters.getPositionAdjustment() ,
        gameParameters.getMaxHeightPixel() - p2.y* gameParameters.getPositionAdjustment());
}
