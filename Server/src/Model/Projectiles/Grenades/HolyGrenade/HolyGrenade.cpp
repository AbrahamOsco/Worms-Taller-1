...
HolyGrenade::HolyGrenade(const GameParameters &gameParameters, const int &waitTime, const TypeFocus &typeFocus)
    ...
    fixedRotation = true;
    this->explodable = Explodable(gameParameters.holyGrenadeGetMainDamage(),
    gameParameters.holyGrenadeGetMaxRadio(), gameParameters.holyGrenadeGetMaxImpulse());
}

void HolyGrenade::getProjectileDTO(std::vector<ProjectileDTO> &vecProjectileDTO) {
    ProjectileDTO aProjectilDTO = ProjectileDTO(PROJ_HOLY_GRENADE,
