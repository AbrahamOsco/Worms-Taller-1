...
ProjectileBazooka::ProjectileBazooka(const GameParameters& gameParameters, const TypeFocus& typeFocus) :
            GameObject(ENTITY_BAZOOKA_PROJECTILE) , gameParameters(gameParameters),
            explodable(gameParameters.getBazookaProjectileDamageMax(), gameParameters.getBazookProjectileRadio(),
            gameParameters.getBazookaProjectilMaxImpulseExplosion() ), aWorld(nullptr) {
    this->typeFocus = typeFocus;
}

void ProjectileBazooka::addToTheWorld(b2World *aWorld, b2Vec2 positionP2,
        const b2Vec2 &impulseProjectile, const float &windValue) {
    ...
    projBazoShape.m_p.Set(0.0f, 0.0f);
